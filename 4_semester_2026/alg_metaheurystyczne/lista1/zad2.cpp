...
LSResult localSearch_Zadanie2(int n, const vector<int>& distMatrix) {
...
int main(){
    ...
    ofstream plik_wynikow("wyniki_zbiorcze_z2.txt");
    ofstream plik_tras("trasy_z2.txt");
    
    for (const string& plik_nazwa : pliki) {
        cout << "[Z2] Plik: " << plik_nazwa << flush;
        ...
        for (int i = 0; i < liczba_prob; ++i) {
            LSResult wynik = localSearch_Zadanie2(n, matrix);
