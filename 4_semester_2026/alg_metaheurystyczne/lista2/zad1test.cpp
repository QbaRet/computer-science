...
SAResult simulatedAnnealing(int n, const vector<int>& distMatrix, double T_start, double alpha, int inner_loops, mt19937& g) {
    ...
    double T = T_start;
    double T_end = 0.001; // Zwykle to zostawiamy bez zmian
...
int main(){
    // Wybieramy TYLKO małe pliki do testów (np. < 300 węzłów, by testy szły w miarę szybko)
    vector<string> pliki = {"dj38.tsp", "wi29.tsp", "qa194.tsp", "uy734.tsp", "zi929.tsp"}; 
    
    // Zestawy parametrów do przetestowania (kombinacje)
    vector<double> T_starts = {1000.0, 10000.0};
    vector<double> alphas = {0.90, 0.95, 0.99, 0.995}; 
    vector<int> multi_inner_loops = {1, 2, 5}; // mnożnik liczby miast (np. n*1, n*2, n*5)
    
    int prob_na_kombinacje = 5; // Ile razy puścić algorytm dla jednej konfiguracji, by wyciągnąć średnią
    ...
    for (const string& plik_nazwa : pliki) {
        ...
        // Przechodzimy przez wszystkie kombinacje parametrów
